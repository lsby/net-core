...
import { 任意接口类型, 接口类型 } from './interface-type'
import { 合并插件结果 } from './plug'

type _F1<A> = A extends 接口类型<infer A1, infer _A2, infer _A3, infer _A4, infer _A5> ? A1 : never
type _F2<A> = A extends 接口类型<infer _A1, infer A2, infer _A3, infer _A4, infer _A5> ? A2 : never
