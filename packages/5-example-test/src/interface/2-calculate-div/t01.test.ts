...
export default new 接口测试(
  ...
  async (中置结果: object): Promise<void> => {
    ...
    let 失败结果校验 = 接口.获得接口返回器().获得接口错误形式Zod().safeParse(中置结果)
    let 正确结果校验 = 接口.获得接口返回器().获得接口正确形式Zod().safeParse(中置结果)
    ...
    assert.equal(正确结果校验.data?.data.result, 5)
  },
)
