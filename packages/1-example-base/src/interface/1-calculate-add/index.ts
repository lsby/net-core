...
// 在实际使用中, 写法会简化, 其他的示例中就会使用更简单的写法

// --------------------
// 1. 定义插件
// --------------------
// 插件 是 接口逻辑 的前置过程
// 在执行 接口逻辑 前, 会依次先执行对应的 插件 们
// 每个 插件 都有返回值, 接口逻辑 会收到所有 插件 的结果的合并值
// 通常插件会用来解析参数等
// 这里实例化了一个内置的 JSON解析参数插件, 该插件会按给定形式解析请求中的JSON参数, 并将其返回为body字段
...
// 3. 定义结果转换器
// --------------------
// 结果转换器 决定如何将 接口逻辑 的返回的 Either值 转换为其他数据结构 的逻辑
// 本例中, 使用内置的封装, 常用结果转换器:
// - 如果是左值, 返回 { status: 'fail', data: ... }
// - 如果是右值, 返回: { status: 'success', data: ... }
let 结果转换器 = new 常用结果转换器(z.never(), z.object({ result: z.number() })) // 第一个参数是错误结果描述, 第二个参是正确结果描述

// --------------------
// 4. 定义结果返回器
// --------------------
// 结果返回器 决定如何将 结果转换器 的结果 返回给请求者 的逻辑
// 本例中, 使用内置的封装, 常用结果返回器:
// - 直接使用 res.send 发送数据
