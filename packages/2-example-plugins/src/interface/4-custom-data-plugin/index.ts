import { 常用结果转换器, 常用结果返回器, 接口, 接口逻辑, 自定义数据插件 } from '@lsby/net-core'
...
// =======================
//
// 自定义数据插件允许直接注入任意的数据到接口逻辑中
// 这不是从请求中解析数据，而是人为地提供数据
//
// 使用场景：
// - 注入配置信息（如系统版本等常量）
// - 测试和 mock（在测试中注入假数据）
...
let 结果转换器 = new 常用结果转换器(z.never(), z.object({ 系统版本: z.string(), 开发者: z.string() }))
