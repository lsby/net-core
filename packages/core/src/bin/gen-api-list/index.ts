...
export async function main(tsconfig路径: string, 目标路径: string, 输出文件路径: string): Promise<void> {
  ...
  let 伴随的虚拟文件们 = 相关源文件们.map((a) => {
    let 代码 = [
      ...
      ``,
      `type 计算结果 = typeof 导入模块 extends { default: infer 默认导出 } ? 默认导出 extends 任意接口 ? true : false : false`,
    ]
  ...
  检查TypeScript诊断(新项目, 伴随的虚拟文件们, '接口列表推导')
  ...
  for (let 源文件 of 伴随的虚拟文件们) {
    ...
    ts.forEachChild(源文件, (node) => {
      if (ts.isTypeAliasDeclaration(node) && node.name.text === '计算结果') {
        ...
        else if (文本结果 !== 'false') throw new Error(`接口列表推导结果不明确: ${源文件.fileName}: ${文本结果}`)
      }
    })
    检查结果.push(结果)
  }
