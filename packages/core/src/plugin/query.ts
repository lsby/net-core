  ...
  public constructor(t: Result) {
    super(错误类型描述, z.object({ query: t }), async (req, _res, 附加参数) => {
      ...
      if (parseResult.success === false) {
        ...
        return new Left({ code: 400, data: format('解析 GET 参数失败: %o', JSON.stringify(parseResult.error)) })
      }
...
export type 合并GET插件结果<Arr extends Array<任意插件>> = Arr extends []
  ? {}
  : Arr extends [infer x, ...infer xs]
    ? x extends infer 插件项
      ? xs extends Array<任意插件>
        ? 插件项 extends 任意GET参数解析插件项
          ? 严格递归合并对象<{ query: 取插件正确ts类型<插件项>['query'] }, 合并GET插件结果<xs>>
          : 合并GET插件结果<xs>
        : {}
      : {}
    : {}
