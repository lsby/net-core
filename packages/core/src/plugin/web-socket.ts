...
import { 任意同步或异步插件, 插件, 解包插件 } from '../interface/interface-plugin'
...
export type 任意异步WS插件 = Task<任意WS插件>
export type 任意同步或异步WS插件 = 任意WS插件 | 任意异步WS插件
export type 任意WS插件项 = 任意WS插件
export type 取WS插件泛型<A> = A extends WebSocket插件<infer x, infer y> ? [x, y] : never
export type 取第一个WS插件输出<Arr extends Array<任意同步或异步插件>> = Arr extends []
  ? {}
  : Arr extends [infer x, ...infer xs]
    ? x extends infer 插件项
      ? xs extends Array<任意同步或异步插件>
        ? 解包插件<插件项> extends 任意WS插件
          ? z.infer<取WS插件泛型<解包插件<插件项>>[0]>
          : 取第一个WS插件输出<xs>
        : {}
      : {}
    : {}
export type 取第一个WS插件输入<Arr extends Array<任意同步或异步插件>> = Arr extends []
  ? {}
  : Arr extends [infer x, ...infer xs]
    ? x extends infer 插件项
      ? xs extends Array<任意同步或异步插件>
        ? 解包插件<插件项> extends 任意WS插件
          ? z.infer<取WS插件泛型<解包插件<插件项>>[1]>
          : 取第一个WS插件输入<xs>
        : {}
      : {}
    : {}
