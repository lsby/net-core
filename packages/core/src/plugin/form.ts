...
export type 合并Form插件结果<Arr extends Array<任意同步或异步插件>> = Arr extends []
  ? {}
  : Arr extends [infer x, ...infer xs]
    ? x extends infer 插件项
      ? xs extends Array<任意同步或异步插件>
        ? 解包插件<插件项> extends infer 实际插件项
          ? 实际插件项 extends 任意Form参数解析插件项
            ? 严格递归合并对象<
                {
                  form: {
                    data: 取插件正确ts类型<实际插件项>['form']['data']
                    files: 取插件正确ts类型<实际插件项>['form']['files']
                  }
                },
                合并Form插件结果<xs>
              >
