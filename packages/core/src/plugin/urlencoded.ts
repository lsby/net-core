...
export class UrlEncoded参数解析插件<Result extends z.AnyZodObject> extends 插件<
  typeof 错误类型描述,
  z.ZodObject<{ urlencoded: Result }>
> {
...
export type 合并UrlEncoded参数解析插件结果<Arr extends Array<任意插件>> = Arr extends []
  ? {}
  : Arr extends [infer x, ...infer xs]
    ? x extends infer 插件项
      ? xs extends Array<任意插件>
        ? 插件项 extends 任意UrlEncoded参数解析插件项
          ? 严格递归合并对象<{ urlencoded: 取插件正确ts类型<插件项>['urlencoded'] }, 合并UrlEncoded参数解析插件结果<xs>>
          : 合并UrlEncoded参数解析插件结果<xs>
        : {}
      : {}
    : {}

export type 计算接口逻辑UrlEncoded参数<接口逻辑> = 合并UrlEncoded参数解析插件结果<获得接口逻辑插件类型<接口逻辑>>
