 ...
 *   - 插件提供错误处理能力, 可以返回带有非2xx状态码的左值来拒绝不合法请求
 *   - 插件失败时服务器直接返回错误，不会进入接口逻辑，也不会执行返回器
 * - 只有所有插件通过后，请求才会进入接口逻辑；后续HTTP状态和响应形式由返回器决定
 * - 常用返回器会以HTTP 200承载接口逻辑的Left和Right，自定义返回器可以采用其他HTTP语义
 */
export class 插件<
